...
#	y_array = np.array([])
	...
	xs = x_array[:]
	ys = y_array[:]
	e_array = (e_array - np.min(e_array))/(np.max(e_array) + np.max(e_array)/4.0-np.min(e_array))
	scatter = ax.scatter(xs, ys,c=e_array[:],cmap=plt.cm.nipy_spectral,s=1)
