...
def z_enc(z):
    ...
    print(z)
    h_ = tf.concat([h,z],axis = 1)
    layer_1 = tf.nn.relu(tf.compat.v1.layers.dense(h_,256, name = "z1"))
    # Hidden fully connected layer with 256 neurons
    layer_2 = tf.nn.relu(tf.compat.v1.layers.dense(layer_1,256, name = "z2"))
    # Output fully connected layer with a neuron for each class
    out_layer =  tf.compat.v1.layers.dense(layer_2,3, name = "z3")
    return out_layer
...
z_vars = [var for var in trainable_vars if 'z' in var.name]
other_vars = [var for var in trainable_vars if 'z' not in var.name]
train_z = optimizer.minimize(loss, name="training_op",  var_list = z_vars)
train_op = optimizer.minimize(loss, name="training_op2",  var_list = other_vars)
init = tf.global_variables_initializer()
...
with tf.Session() as sess:
	...
	for epoch in range(10000):
		...
		batch_x, batch_y = set_batch(batch_size,maxx)
		print(maxx)
#		z = np.reshape(np.array((random.uniform(0, 1),random.uniform(0, 1))),(-1,2))
		z = np.reshape(np.random.uniform(low=-0.1, high=0.1, size=(100,10)),(-1,10))
	#	_, c = sess.run([train_op, loss], feed_dict={X: batch_x,
                    #                                        Y: batch_y, Z:z})
		
		v = 0
		for p in range(3):
			_, c = sess.run([train_z, loss], feed_dict={X: batch_x,
                                                            Y: batch_y, Z:z})
			print(c)
		'''
