...
x_array = np.array([])
y_array = np.array([])
dir_array_y = np.array([])
dir_array_x = np.array([])
...
#		print(mom)
			...
			print("z")
			print(z)
			c_ = sess.run([y_], feed_dict={X: batch_x, Z:z})
			print("y_pred")
			print(c_)
			z_old = np.copy(z)
...
#	y_array = np.array([])
		...
		y_array = np.append(y_array, c_[0][:,0])
		direction = c[0] - c_[0] 
		dir_array_y = np.append(dir_array_y, direction[:,0])
		dir_array_x = np.append(dir_array_x, np.zeros((3,1)))
		print("arrows")
		print(dir_array_y )
		print(dir_array_x)
		print("end_arrows")
...
#	ax.scatter(xs, ys,color='red',s=1)
	ax.quiver(xs,ys,dir_array_x[:],dir_array_y[:])
	...
	ax.set_ylabel('Y Label')
        ...
        #plt.axis('off')
	plt.savefig("./pred_arrow3.png", bbox_inches='tight')
