...
#			print(type(z))
	...
	for l in range(100):
		...
		if l < 20:
			...
			print(z_diff) 
			print(np.abs(z_diff))
			low_vals = np.abs(z_diff) < 0.03
			print(low_vals) 
		#	z[low_vals] = z[low_vals] + z_diff[low_vals] *( -np.log(5*(np.abs(z_diff[low_vals]))) + 0.7)

			z[low_vals] = z[low_vals] + z_diff[low_vals]/np.abs(z_diff[low_vals])*0.05
			print("z_new_changed")
			print(z)
		c = sess.run([y_], feed_dict={X: batch_x, Z:z})
		print("x")
